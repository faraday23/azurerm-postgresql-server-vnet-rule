...
resource "azurerm_postgresql_database" "db" {
  ...
  name                = "db-${var.names.product_name}-${var.names.environment}-${var.srvr_id}"
  resource_group_name = var.resource_group_name
...
resource "azurerm_postgresql_virtual_network_rule" "vn_rule01" {
  ...
  name                = azurerm_postgresql_server.primary.name-"${var.srvr_id}"
  resource_group_name = var.resource_group_name
  server_name         = azurerm_postgresql_server.primary.name
...
resource "azurerm_postgresql_virtual_network_rule" "vn_rule02" {
  ...
  name = "${var.subnet_name_replica}-${var.srvr_id_replica}"
  resource_group_name = var.vnet_replica_resource_group_name
  server_name         = azurerm_postgresql_server.replica.0.name
